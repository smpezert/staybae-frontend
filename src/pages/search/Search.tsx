...
const Search = () => {
    ...
    const { location: searchLocation, startDate, endDate, region, numberOfGuests } = location.state;
    ...
    return (
        <div className="flex pt-10">
            <section className="flex-grow px-6">
                <p className="text-xs">{searchResults.length} Stay(s) - {range} - for {numberOfGuests} guest(s)</p>

                <h1 className="text-3xl font-semibold mt-2 mb-6">Stays in {searchLocation ?? region}</h1>
                ...
                <div className="flex flex-col">
                    {searchResults.length === 0 &&
                        <span className="text-md">There are no results matching your query.</span>}
